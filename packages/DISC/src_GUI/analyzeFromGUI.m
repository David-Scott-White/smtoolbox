function [data, disc_input] = analyzeFromGUI(data, disc_input, indices, analyze_all, popup)
...
modCentroid = 0;
% if isfield(data, 'alignTransform')
%     if ~isempty(data.alignTransform)
%         modCentroid = 1;
%         maxdist = mean(data.alignTransform.rmse)*2.5;
%     end
% end
% modCentroid = 0;
% maxdist = 1.5;
    ...
    function goAnalyze(~,~)
        ...
        if ~analyze_all
            data.rois(roi_idx, ch_idx).disc_fit =  ...
                runDISC(data.rois(roi_idx, ch_idx).time_series, disc_input);
            if modCentroid
                % maxdist = mean(data.alignTransform.rmse)*2;
                data.rois(roi_idx,ch_idx) = ...
                    modifyEventsByCentroid(data.rois(roi_idx,ch_idx), maxdist, 1);
            end
        ...
        elseif analyze_all
            ...
            for ii = 1:n_rois
                ...
                try
                    ...
                    if modCentroid
                        data.rois(ii,ch_idx) = ...
                            modifyEventsByCentroid(data.rois(ii,ch_idx), maxdist, 0);
                    end
                catch ME
